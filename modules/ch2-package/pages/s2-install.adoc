...
.. Install the container tools packages, if they are not already installed on your _test machine_. They are _not_ strictly required to run MicroShift, but they do provide some troubleshooting tools which can be valuable in a development environment.
+
...
--
+
Encoded in the `auth` field of that pull secret are the `microshift` username and `redhat123` password.
...
.. Check that you have two CPU cores and at least 2GB of memory. If you are using the classroom environment, you have more memory than required on the `serverb` VM.
+
[source,subs="verbatim,quotes"]
--
$ *nproc*
...
.. Check all pods running on MicroShift. It may take a while, so repeat the following command until all pods are ready and running.
+
WARNING: Some pods may enter a error state for a while, and recover by themselves after a few minutes. Do not panic if you see a status of `CrashLoopBackOff` or `Error`. 
+
[source,subs="verbatim,quotes"]
--
...
+
[ PODS ARE NOT RECOVERING :-( but node reports ready ]
+
[source,subs="verbatim,quotes"]
--
...
IMPORTANT: The way we configured MicroShift, it does _not_ contain a pull secret to download container images from Red Hat, even if it has access to the Internet. It can only pull containers from its mirror registry, which is usually what organizations with stricty security policies demand.
