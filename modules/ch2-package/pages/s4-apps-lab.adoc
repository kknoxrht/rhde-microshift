...
.. Return to the first terminal, running the `oc port-forward` command, and terminate the networkl tunnel with kbd:[Ctrl+C].
...
.. Edit the `/etc/hosts` file to map the host name of the route to the IP address of your _test machine_ by appending the following line:
+
[source,subs="verbatim,quotes"]
--
...
[source,subs="verbatim,quotes"]
--
$ *oc patch service hellophp --type json --patch '[{"op": "replace", "path": "/spec/allocateLoadBalancerNodePorts", "value": false }]'*
service/hellophp patched
$ *oc patch service hellophp --type json --patch '[{"op": "remove", "path": "/spec/ports/0/nodePort"}]'
service/hellophp patched*
$ *oc get service*
NAME       TYPE           CLUSTER-IP    EXTERNAL-IP       PORT(S)    AGE
hellophp   LoadBalancer   10.43.25.48   192.168.122.192   8080/TCP   13m
--
+
